...
void BFS(int root,int depth){
    vector<int> rChildren = Rgraph[root];
    depthTree[root] = depth;
    for( int child : rChildren){
...
int main(){
    ...
    fore(_,0,q){
        ...
        if(from == to){
            res = 0;
        }else if(graph[from] == to){
            res = 1;
        }
        else if( idCycle[from] != -1&& idCycle[from] == idCycle[to]){ //Caso estan en el mismo ciclo
            //Si from esta antes en el orden, basta con avanzar
            if(depthCycle[from] < depthCycle[to]){
                res = depthCycle[to] - depthCycle[from];
            //Si esta despues, hay que "dar toda la vuelta" y volver para atras la diferencia en pasitos
            }else{
                res = cycleLength[idCycle[from]] - (depthCycle[from] - depthCycle[to]);
            }
        }else{ //O estan en el mismo arbol o uno esta en el ciclo y otro en el arbol
           
           int TreeRoot = computeLift(from,depthTree[from]);
           int candidate = computeLift(from,depthTree[from] - depthTree[to]);
           if(candidate == to) res = depthTree[from] - depthTree[to];
           else if(idCycle[TreeRoot] == idCycle[to]){
                
            //Si from esta antes en el orden, basta con avanzar
            if(depthCycle[TreeRoot] < depthCycle[to]){
                res = depthTree[from] + depthCycle[to] - depthCycle[TreeRoot];
            //Si esta despues, hay que "dar toda la vuelta" y volver para atras la diferencia en pasitos
            }else{
                res = depthTree[from] + cycleLength[idCycle[TreeRoot]] - (depthCycle[TreeRoot] - depthCycle[to]);
            }
           }
